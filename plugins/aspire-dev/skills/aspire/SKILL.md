---
name: aspire
description: "**WORKFLOW SKILL** - Orchestrates Aspire applications using the Aspire CLI and MCP tools for running, debugging, and managing distributed apps. USE FOR: aspire run, aspire stop, start aspire app, aspire describe, list aspire integrations, debug aspire issues, view aspire logs, add aspire resource, aspire dashboard, update aspire apphost. DO NOT USE FOR: non-Aspire .NET apps (use dotnet CLI), container-only deployments (use docker/podman). INVOKES: Aspire MCP tools (list_resources, list_integrations, list_structured_logs, get_doc, search_docs), bash for CLI commands. FOR SINGLE OPERATIONS: Use Aspire MCP tools directly for quick resource status or doc lookups."
---

# Aspire Skill

This skill provides guidance for working with .NET Aspire applications. Aspire is an orchestrator for distributed applications that configures dependencies, builds, and runs all resources. Resources are defined in an AppHost project's `Program.cs` (commonly referred to as `apphost.cs`), including application code and external dependencies.

## General recommendations for working with Aspire

1. Before making any changes always run the apphost using `aspire run` and inspect the state of resources to make sure you are building from a known state.
2. Changes to the AppHost code will require a restart of the application to take effect.
...
If there is already an instance of the application running it will prompt to stop the existing instance. You only need to restart the application if the AppHost code is changed, but if you experience problems it can be useful to reset everything to the starting state.

## Environment Diagnostics

Run `doctor` first when encountering environment issues (missing SDKs, wrong versions, etc.). It performs comprehensive checks and returns actionable fix suggestions.
...
When resources are stuck in "Starting" with no errors in Aspire logs, the issue is likely at the DCP layer. Use the `debug-dcp` skill for DCP environment variables, log analysis, and known failure patterns.

## Health Check Gotchas

- `WithHttpHealthCheck` uses a default HttpClient with no TLS bypass -- fails silently with self-signed certs (K3s, etc.)
- `WaitFor()` blocks `StartAsync` until health checks pass -- a broken health check blocks ALL dependent resources
- For containers with self-signed TLS, use file-based health checks (e.g., kubeconfig existence) instead of HTTP
...
## Creating Custom Hosting Integrations

When built-in integrations don't meet your needs, create custom hosting integrations. Use `search_docs` with "custom integrations" or `get_doc` with slug "custom-integrations" for official guidance.

### Custom Resource Types

Extend `ContainerResource` for container-based resources:

```csharp
namespace Aspire.Hosting.ApplicationModel;

public class MyResource(string name) : ContainerResource(name), IResourceWithConnectionString
{
    internal const string EndpointName = "http";
    public EndpointReference PrimaryEndpoint => new(this, EndpointName);

    public ReferenceExpression ConnectionStringExpression =>
        ReferenceExpression.Create($"Host={PrimaryEndpoint.Property(EndpointProperty.Host)};Port={PrimaryEndpoint.Property(EndpointProperty.Port)}");
}
```
...
{
    // AddXxx - Adds the resource
    public static IResourceBuilder<MyResource> AddMyResource(
        this IDistributedApplicationBuilder builder, string name, int? port = null)
    {
        var resource = new MyResource(name);
        return builder.AddResource(resource)
            .WithImage("myimage").WithImageTag("latest")
            .WithHttpEndpoint(port: port, targetPort: 8080, name: MyResource.EndpointName);
    }

    // WithXxx - Configures the resource
    public static IResourceBuilder<MyResource> WithCustomOption(
        this IResourceBuilder<MyResource> builder, string value)
    {
        builder.Resource.CustomProperty = value;
        return builder;
    }
}
```
...
1. **Hosting Integration** (`YourCompany.Aspire.Hosting.MyService`) -- Resource type + AddXxx/WithXxx extensions, referenced by AppHost
2. **Client Integration** (`YourCompany.Aspire.MyService.Client`) -- Service registration + health checks, referenced by consuming services
